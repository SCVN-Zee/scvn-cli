...
async function restageUnionIfStale(
  ...
  const currentlyStagedAddons = present.filter((pkg) => pkg !== CORE_PKG && pkg !== PPX_PKG);
...
export async function reconfigureMcp(opts: ReconfigureMcpOpts): Promise<void> {
  ...
  const cli = await ensureUnityMcpCli(candidateCore, opts).catch((err: unknown) => {
    log(
      opts,
      "warn",
      `unity-mcp-cli unavailable (${err instanceof Error ? err.message : String(err)}) — ` +
  ...
  // 4. .mcp.json — warn-only: vendoring already succeeded.
  if (cli !== null) {
